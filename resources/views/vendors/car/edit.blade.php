                        ...
                        <div class="version-body {{ $language->direction == 1 ? 'rtl text-right' : '' }}">
                          <div class="row">
                            ...
                            <div class="col-lg-4">
                              <div class="form-group {{ $language->direction == 1 ? 'rtl text-right' : '' }}">
                                @php
                                  $brand_models = App\Models\Car\CarModel::where(
                                      'brand_id',
                                      $carContent ? $carContent->brand_id : 0,
                                  )
                                      ->where('status', 1)
                                      ->get();
                                @endphp
                            ...
                            <div class="col-lg-4">
                              <div class="form-group {{ $language->direction == 1 ? 'rtl text-right' : '' }}">
                                @php
                                  $transmission_types = App\Models\Car\TransmissionType::where(
                                      'language_id',
                                      $language->id,
                                  )
                                      ->where('status', 1)
                              ...
                              <div class="col-lg-4">
                                <div class="form-group {{ $language->direction == 1 ? 'rtl text-right' : '' }}">
                                  ...
                                  <select name="{{ $language->code }}_country_id"
                                    class="form-control js-example-basic-single8" data-code="{{ $language->code }}">
                                    <option selected disabled>{{ __('Select a country') }}</option>

                                    @foreach ($Countries as $Country)
                                      <option @selected(@$carContent->country_id == $Country->id) value="{{ $Country->id }}">
                                        {{ $Country->name }}</option>
                                    @endforeach
                                  </select>
                                </div>
                            ...
                            @php
                              ...
                              $Stateshave = App\Models\Location\State::where([
                                  ...
                                  ['country_id', $carContent ? $carContent->country_id : 0],
                              ])->get();
                                ...
                                <div class="form-group {{ $language->direction == 1 ? 'rtl text-right' : '' }}">
                                  ...
                                  <select name="{{ $language->code }}_state_id"
                                    ...
                                    @foreach ($States as $State)
                                      <option @selected(@$carContent->state_id == $State->id) value="{{ $State->id }}">
                                        {{ $State->name }}</option>
                                    @endforeach
                                  </select>
                                </div>
                              ...
                              <div class="form-group {{ $language->direction == 1 ? 'rtl text-right' : '' }}">
                                ...
                                <select name="{{ $language->code }}_city_id"
                                  ...
                                  @foreach ($cities as $City)
                                    <option @selected(@$carContent->city_id == $City->id) value="{{ $City->id }}">
                                      {{ $City->name }}</option>
                                  @endforeach
                                </select>
                              </div>
                            ...
                            <div class="col-lg-12">
                              <div class="form-group">
                                ...
                                <input type="text" name="{{ $language->code }}_address" class="form-control"
                                  ...
                                  value="{{ $carContent ? $carContent->address : '' }}">
                                @if ($carContent)
                            ...
                            @if ($language->is_default == 1 && $settings->google_map_api_key_status == 1)
                              <div class="col-lg-6">
                                <div class="form-group ">
                                  <label>{{ __('Latitude') . '*' }}</label>
                                  <input type="text" class="form-control" value="{{ $car->latitude }}"
                              ...
                              <div class="col-lg-6">
                                <div class="form-group ">
                                  <label>{{ __('Longitude') . '*' }}</label>
                                  <input type="text" class="form-control" value="{{ $car->longitude }}"
                          ...
                          @foreach ($specifications as $specification)
                            <tr>
                              <td>
                                @foreach ($languages as $language)
                                  @php
                                    $sp_content = App\Models\Car\CarSpecificationContent::where([
                                        ['language_id', $language->id],
                                        ['car_specification_id', $specification->id],
                                    ])->first();
                                  @endphp
                                  <div class="form-group">
...
@section('script')
  ...
  @endif
  <script>
    var address = "{{ $carAddress }}";
    var labels = "{!! $labels !!}";
    var values = "{!! $values !!}";
    var getBrandUrl = "{{ route('vendor.get-car.brand.model') }}";
    var getStateUrl = "{{ route('vendor.car_management.get-state') }}";
    var getCityUrl = "{{ route('vendor.car_management.get-city') }}";
  </script>
