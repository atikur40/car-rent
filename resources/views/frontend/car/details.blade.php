          ...
          <div class="product-single-details">
            <div class="row" data-aos="fade-up">
              <div class="col-md-8">
                ...
                @php
                  if ($car->car_content->first()->city_id) {
                      $city = App\Models\Location\City::Where('id', $car->car_content->first()->city_id)->first()->name;
                  }
                  if ($car->car_content->first()->state_id) {
                      $State = App\Models\Location\State::Where('id', $car->car_content->first()->state_id)->first()
                          ->name;
                  }
                  if ($car->car_content->first()->country_id) {
                      $country = App\Models\Location\Country::Where(
                          'id',
                          $car->car_content->first()->country_id,
                      )->first()->name;
                  }

                @endphp
                <p class="address"><i class="fal fa-map-marker-alt"></i> {{ @$city }}
                  @if (@$State)
                    ,{{ $State }}
                    @endif @if (@$country)
                      ,{{ @$country }}
                  ...
                  <div class="row">
                    ...
                    @if (count($specifications) > 0)
                      @foreach ($specifications as $specification)
                        @php
                          $car_specification_content = App\Models\Car\CarSpecificationContent::where([
              ...
              @if ($car->vendor_id != 0)
                <div class="user mb-20">
                  ...
                  <div class="user-info">
                    ...
                    <p class="font-sm mb-1">{{ @$car->vendor->vendor_info->city }}
                      , {{ @$car->vendor->vendor_info->country }}</p>
                  </div>
                </div>
              @else
                <div class="user mb-20">
