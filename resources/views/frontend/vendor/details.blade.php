          ...
          <div class="vendor-info">
            ...
            <span class="text-light d-block mt-2">{{ __('Total Cars') . ' : ' }}
              @if (request()->filled('admin'))
                @php
                  $total_cars = App\Models\Car::where('vendor_id', 0)->get()->count();
                @endphp
                {{ $total_cars }}
              @else
              ...
              <div class="row">
                @if (count($all_cars) > 0)
                  @foreach ($all_cars as $all_car)
                    @php
                      $car_content = App\Models\Car\CarContent::where([
              ...
              <ul class="toggle-list list-unstyled mt-15" id="toggleList" data-toggle-show="6">
                ...
                @if (request()->input('admin') != true)
                  @if (!is_null(@$vendorInfo->city))
                    @php
                      $cityName = \App\Models\Location\City::where('id', @$vendorInfo->city)
                          ->where('language_id', $language->id)
                          ->value('name');
                    @endphp
                    <li>
                      <span class="first">{{ __('City') . ':' }}</span>
                      <span class="last">{{ $cityName ?? @$vendorInfo->city }}</span>
                    </li>
                  @endif
                  ...
                  @if (!is_null(@$vendorInfo->state))
                    @php
                      $stateName = \App\Models\Location\State::where('id', @$vendorInfo->state)
                          ->where('language_id', $language->id)
                          ->value('name');
                    @endphp
                    <li>
                      <span class="first">{{ __('State') . ':' }}</span>
                      <span class="last">{{ $stateName ?? @$vendorInfo->city }}</span>
                    </li>
                  @endif

                  @if (!is_null(@$vendorInfo->country))
                    @php
                      $countryName = \App\Models\Location\Country::where('id', @$vendorInfo->country)
                          ->where('language_id', $language->id)
                          ->value('name');
                    @endphp
                    <li>
                      <span class="first">{{ __('Country') . ':' }}</span>
                      <span class="last">{{ $countryName ?? @$vendorInfo->city }}</span>
                    </li>
                  @endif
                @endif
